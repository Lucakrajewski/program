# Feature Index

> Central tracking for all features. Updated by skills automatically.

## Status Legend
- **Planned** - Requirements written, ready for development
- **In Progress** - Currently being built
- **In Review** - QA testing in progress
- **Deployed** - Live in production

## Features

| ID | Feature | Status | Spec | Created |
|----|---------|--------|------|---------|
| PROJ-1 | Onboarding & Gemeinde-Auswahl | In Progress | [PROJ-1-onboarding-gemeinde-auswahl.md](PROJ-1-onboarding-gemeinde-auswahl.md) | 2026-03-11 |
| PROJ-2 | Mängelmelder | In Progress | [PROJ-2-maengelmelder.md](PROJ-2-maengelmelder.md) | 2026-03-11 |
| PROJ-3 | News & Events | In Progress | [PROJ-3-news-events.md](PROJ-3-news-events.md) | 2026-03-11 |
| PROJ-4 | Rathaus-Chat | In Progress | [PROJ-4-rathaus-chat.md](PROJ-4-rathaus-chat.md) | 2026-03-11 |
| PROJ-5 | Admin-Portal Grundfunktionen | In Progress | [PROJ-5-admin-portal-grundfunktionen.md](PROJ-5-admin-portal-grundfunktionen.md) | 2026-03-11 |

<!-- Add features above this line -->

## Next Available ID: PROJ-6

---

## Phase-Übersicht

### Phase 1 — MVP (PROJ-1 bis PROJ-5)
Ziel: Erste Pilot-Gemeinde live bringen

| ID | Feature | Priorität |
|----|---------|-----------|
| PROJ-1 | Onboarding & Gemeinde-Auswahl | P0 |
| PROJ-2 | Mängelmelder | P0 |
| PROJ-3 | News & Events | P0 |
| PROJ-4 | Rathaus-Chat | P0 |
| PROJ-5 | Admin-Portal Grundfunktionen | P0 |

### Phase 2 — Erweiterung (geplant)
- PROJ-6: Terminbuchung (P1)
- PROJ-7: Müllkalender adressbasiert (P1)
- PROJ-8: Bürgerbeteiligung — Umfragen & Ideen (P1)
- PROJ-9: Lokale Wirtschaft — Branchenverzeichnis (P1)
- PROJ-10: Jobs & Ehrenamt (P1)
- PROJ-11: Community — Schwarzes Brett (P1)
- PROJ-12: Nachhaltigkeit & Infrastruktur (P1)

### Phase 3 — Differenzierung (geplant)
- PROJ-13: Tourismus-Modul (P2)
- PROJ-14: Gamification (P2)
- PROJ-15: KI-Modul — RAG & Antwortvorschläge (P3)
